...
extern "C" char *fd2data(struct Fd *fd);
extern "C" uint64_t fd2num(struct Fd *fd);
extern "C" int fd_alloc(struct Fd **fd_store);
extern "C" int fd_close(struct Fd *fd, bool must_exist);
extern "C" int fd_lookup(int fdnum, struct Fd **fd_store);
extern "C" int dev_lookup(int devid, struct Dev **dev_store);
